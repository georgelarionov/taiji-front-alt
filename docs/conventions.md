...
- **`bigLinks: NavLink[]`** — крупные ссылки (Новости / Анонсы / Медиа-архив / Контакты).
...
**Hover-подчёркивание ссылок** (`.link-underline` / `.link-underline-group` в `global.css` —
крошки, «Подробнее» в карточках, «Скачать PDF»): входит слева направо, **уходит вправо** (а не
обратно влево). Реализовано сменой `transform-origin`: покой → `right` + `scaleX(0)`, ховер →
`left` + `scaleX(1)`. Единый источник; новые подчёркивания делать так же. Шапка использует свою
`after:`-анимацию (Tailwind-классы в `Header.astro`), к этим утилитам не относится.
