...
- **Header** (`Header.astro`) — сейчас вызывается из `Hero` через `slot="header"` и реализует
  **только `variant="overlay"`** (светлая шапка поверх hero). `variant` заложен в Props на
  будущее; **solid-вариант** (на сплошном фоне, для внутренних страниц без hero) **пока не
  реализован — добавлять вместе с дизайном** (YAGNI).
...
- **Solid-вариант Header** под внутренние страницы (без hero) — задел в Props
  (`variant?: "overlay"`), реализации нет; добавить, когда появится дизайн.
